...
class Map2 extends Component {
  ...
  init(id) {
    ...
    let map = L.map(id, config.map2.params);

    L.control.zoom({ position: "topleft"}).addTo(map);
    L.control.scale({ position: "bottomleft"}).addTo(map);

    const tileLayer = L.tileLayer(config.map2.tileLayer.uri, config.map2.tileLayer.params).addTo(map);
