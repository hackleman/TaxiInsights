...
class Map4 extends Component {
  ...
  init(id) {
    ...
    let map = L.map(id, config.map4.params);

    L.control.zoom({ position: "topleft"}).addTo(map);
    L.control.scale({ position: "bottomleft"}).addTo(map);

    const tileLayer = L.tileLayer(config.map4.tileLayer.uri, config.map4.tileLayer.params).addTo(map);
